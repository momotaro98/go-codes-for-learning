...
func testUserOneToOneUserlocationUsingUserIdUserlocation(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &foreign, userlocationDBTypes, true, userlocationColumnsWithDefault...); err != nil {
	...
	check, err := local.UserIdUserlocation().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadUserIdUserlocation(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UserIdUserlocation == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.UserIdUserlocation = nil
	if err = local.L.LoadUserIdUserlocation(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UserIdUserlocation == nil {
		t.Error("struct should have been eager loaded")
...
func testUserOneToOneSetOpUserlocationUsingUserIdUserlocation(t *testing.T) {
	var err error
	...
	for i, x := range []*Userlocation{&b, &c} {
		err = a.SetUserIdUserlocation(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.UserIdUserlocation != x {
		...
		if exists, err := UserlocationExists(ctx, tx, x.UserId); err != nil {
			t.Fatal(err)
		} else if !exists {
...
func testUserToOnePositionUsingPositionId(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := local.PositionId().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&local}
	if err = local.L.LoadPositionId(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.PositionId == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.PositionId = nil
	if err = local.L.LoadPositionId(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.PositionId == nil {
		t.Error("struct should have been eager loaded")
...
func testUserToOneSetOpPositionUsingPositionId(t *testing.T) {
	var err error
	...
	for i, x := range []*Position{&b, &c} {
		err = a.SetPositionId(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToOneRemoveOpPositionUsingPositionId(t *testing.T) {
	var err error
	...
	if err = a.SetPositionId(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemovePositionId(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.PositionId().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
...
var (
	userDBTypes = map[string]string{`UserId`: `char`, `Name`: `varchar`, `Email`: `varchar`, `NickName`: `varchar`, `Sex`: `char`, `Birthday`: `date`, `PhotoUrl`: `text`, `PositionId`: `smallint`, `AcademicBackground`: `text`, `Company`: `text`, `SelfIntroduction`: `text`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_           = bytes.MinRead
)
