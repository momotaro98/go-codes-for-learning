...
type User struct {
	...
	PositionId         null.Int16  `boil:"positionId" json:"positionId,omitempty" toml:"positionId" yaml:"positionId,omitempty"`
	AcademicBackground null.String `boil:"academicBackground" json:"academicBackground,omitempty" toml:"academicBackground" yaml:"academicBackground,omitempty"`
	Company            null.String `boil:"company" json:"company,omitempty" toml:"company" yaml:"company,omitempty"`
	SelfIntroduction   null.String `boil:"selfIntroduction" json:"selfIntroduction,omitempty" toml:"selfIntroduction" yaml:"selfIntroduction,omitempty"`
...
var UserWhere = struct {
	...
	PositionId         whereHelpernull_Int16
	AcademicBackground whereHelpernull_String
	Company            whereHelpernull_String
	SelfIntroduction   whereHelpernull_String
	CreatedAt          whereHelpertime_Time
	UpdatedAt          whereHelpertime_Time
}{
	UserId:             whereHelperstring{field: "`users`.`userId`"},
	...
	PhotoUrl:           whereHelpernull_String{field: "`users`.`photoUrl`"},
	PositionId:         whereHelpernull_Int16{field: "`users`.`positionId`"},
	AcademicBackground: whereHelpernull_String{field: "`users`.`academicBackground`"},
	Company:            whereHelpernull_String{field: "`users`.`company`"},
	SelfIntroduction:   whereHelpernull_String{field: "`users`.`selfIntroduction`"},
	CreatedAt:          whereHelpertime_Time{field: "`users`.`createdAt`"},
	UpdatedAt:          whereHelpertime_Time{field: "`users`.`updatedAt`"},
}

// UserRels is where relationship names are stored.
var UserRels = struct {
	PositionId         string
	UserIdUserlocation string
}{
	PositionId:         "PositionId",
	UserIdUserlocation: "UserIdUserlocation",
}

// userR is where relationships are stored.
type userR struct {
	PositionId         *Position
	UserIdUserlocation *Userlocation
}
...
var (
	userAllColumns            = []string{"userId", "name", "email", "nickName", "sex", "birthday", "photoUrl", "positionId", "academicBackground", "company", "selfIntroduction", "createdAt", "updatedAt"}
	userColumnsWithoutDefault = []string{"userId", "name", "email", "nickName", "birthday", "photoUrl", "positionId", "academicBackground", "company", "selfIntroduction"}
	userColumnsWithDefault    = []string{"sex", "createdAt", "updatedAt"}
...
// PositionId pointed to by the foreign key.
func (o *User) PositionId(mods ...qm.QueryMod) positionQuery {
	queryMods := []qm.QueryMod{
		qm.Where("`positionId` = ?", o.PositionId),
	}
...
// UserIdUserlocation pointed to by the foreign key.
func (o *User) UserIdUserlocation(mods ...qm.QueryMod) userlocationQuery {
	queryMods := []qm.QueryMod{
	...
	queries.SetFrom(query.Query, "`userlocations`")

	return query
}

// LoadPositionId allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (userL) LoadPositionId(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`positions`), qm.WhereIn(`positions.positionId in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		...
		object.R.PositionId = foreign
		if foreign.R == nil {
			foreign.R = &positionR{}
		}
		foreign.R.PositionIdUsers = append(foreign.R.PositionIdUsers, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.PositionId, foreign.PositionId) {
				local.R.PositionId = foreign
				if foreign.R == nil {
					foreign.R = &positionR{}
				}
				foreign.R.PositionIdUsers = append(foreign.R.PositionIdUsers, local)
				break
			}
		}
	}
...
// LoadUserIdUserlocation allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-1 relationship.
func (userL) LoadUserIdUserlocation(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`userlocations`), qm.WhereIn(`userlocations.userId in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for userlocations")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for userlocations")
	}
	...
	if singular {
		foreign := resultSlice[0]
		object.R.UserIdUserlocation = foreign
		if foreign.R == nil {
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserId == foreign.UserId {
				local.R.UserIdUserlocation = foreign
				if foreign.R == nil {
					foreign.R = &userlocationR{}
				}
...
// SetPositionId of the user to the related item.
// Sets o.R.PositionId to related.
// Adds o to related.R.PositionIdUsers.
func (o *User) SetPositionId(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Position) error {
	var err error
	if insert {
	...
	values := []interface{}{related.PositionId, o.UserId}
	...
	if o.R == nil {
		...
		o.R.PositionId = related
	}

	if related.R == nil {
		related.R = &positionR{
			PositionIdUsers: UserSlice{o},
		}
	} else {
		related.R.PositionIdUsers = append(related.R.PositionIdUsers, o)
	}

	return nil
}

// RemovePositionId relationship.
// Sets o.R.PositionId to nil.
// Removes o from all passed in related items' relationships struct (Optional).
func (o *User) RemovePositionId(ctx context.Context, exec boil.ContextExecutor, related *Position) error {
	var err error

	queries.SetScanner(&o.PositionId, nil)
	if _, err = o.Update(ctx, exec, boil.Whitelist("positionId")); err != nil {
		return errors.Wrap(err, "failed to update local table")
	}
	...
	for i, ri := range related.R.PositionIdUsers {
		if queries.Equal(o.PositionId, ri.PositionId) {
			continue
		}

		ln := len(related.R.PositionIdUsers)
		if ln > 1 && i < ln-1 {
			related.R.PositionIdUsers[i] = related.R.PositionIdUsers[ln-1]
		}
		related.R.PositionIdUsers = related.R.PositionIdUsers[:ln-1]
		break
	}
	return nil
}

// SetUserIdUserlocation of the user to the related item.
// Sets o.R.UserIdUserlocation to related.
// Adds o to related.R.UserId.
func (o *User) SetUserIdUserlocation(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Userlocation) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			UserIdUserlocation: related,
		}
	} else {
		o.R.UserIdUserlocation = related
	}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, userId string, selectCols ...string) (*User, error) {
	...
	query := fmt.Sprintf(
		"select %s from `users` where `userId`=?", sel,
	)
